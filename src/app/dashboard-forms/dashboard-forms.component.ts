...
export class DashboardFormsComponent implements OnInit {
  ...
  shouldShowButtons(): boolean {
    const networkExtensionRequired = this.networkExtensionForm.get('isNetworkExtensionRequired')?.value;
    const isMeterChangeReq = this.networkExtensionForm.get('isMeterChangeReq')?.value;
    const applicationTypeCode = this.data?.serviceRegistration?.applicationTypeCode;

    // if (this.discom == 1) {
    //   return false;
    // }

    if ((applicationTypeCode === 'LE' || applicationTypeCode === 'LR') && isMeterChangeReq === '1') {
      return true;
    }
    return true;
  }
  ...
  async submitSiteInspectionForm() {
    ...
    //   ) {
    //     this.snackBar
    //       .open('inspection date required', 'OK', { verticalPosition: 'top' })
    //       .onAction()
    //       .subscribe(() => {
