...
@Component({
  selector: 'app-smr-estimation-change',
  templateUrl: './smr-estimation-change.component.html',
  styleUrls: ['./smr-estimation-change.component.scss']
})
export class SmrEstimationChangeComponent implements OnInit {
  data: any = null;
  accountId: string = ''; 
  ...
  async getDetails() {
    try {
      ...
      const response = await this.fieldactivityService.getWrongEstimationDataByReferenceNoForSMR(detailsFilter);
  ...
  async chnageInApprovalAuthority() {
    ...
    try {
      ...
      this.saveData = await this.fieldactivityService.forSingleMeterReplacement(detailsFilter);
