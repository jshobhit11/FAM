...
@Component({
  selector: 'app-work-improvement-to-estimate',
  templateUrl: './work-improvement-to-estimate.component.html',
  styleUrls: ['./work-improvement-to-estimate.component.scss']
})
export class WorkImprovementToEstimateComponent implements OnInit {
  ...
  async getDetails() {
    try {
      ...
      const response = await this.fieldactivityService.getWrongWImprovementDataByWorkOrderNo(detailsFilter);
  ...
  async chnageStatusToEstimation() {
    ...
    this.loader.show('Updating Estimation...');
    try {
      ...
      const userName = sessionStorage.getItem('user-name');
      const userCode = sessionStorage.getItem('user-code');
      const detailsFilter: any = {apiKey,serviceKey,userRole,userName,userCode,serviceRegistrationsId:this.data?.serviceRegistrationsId};
      this.saveData = await this.fieldactivityService.forWorkImprovementsToEstimate(detailsFilter);
