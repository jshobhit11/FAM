...
@Component({
  selector: 'app-service-main-estimate-far',
  templateUrl: './service-main-estimate-far.component.html',
  styleUrls: ['./service-main-estimate-far.component.scss']
})
export class ServiceMainEstimateFarComponent implements OnInit {
  data: any = null;
  accountId: string = ''; 
  ...
  async getDetails() {
    try {
      ...
      const response = await this.fieldactivityService.getDateByWorkOrderNoForServiceMainEstimate(detailsFilter);
  ...
  async chnageStatusToEstimation() {
    ...
    this.loader.show('Reverting to Service Main Estimate...');
    try {
      ...
      this.saveData = await this.fieldactivityService.putBackToServiceMainEstimate(detailsFilter);
