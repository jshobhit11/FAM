...
@Component({
  selector: 'app-line-ext-change',
  templateUrl: './line-ext-change.component.html',
  styleUrls: ['./line-ext-change.component.scss']
})
export class LineExtChangeComponent implements OnInit {
  data: any = null;
  accountId: string = '';
  ...
  async getDetails() {
    try {
      ...
      const response =
        await this.fieldactivityService.getDataByWorkorderNoMoveBackToWorkExecutionForLineExt(
          detailsFilter
        );
  ...
  async putBackToWorkExecution() {
    try {
      ...
      this.saveData = await this.fieldactivityService.putBackToWorkExecution(
        detailsFilter
      );
