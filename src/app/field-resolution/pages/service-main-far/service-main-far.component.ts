...
@Component({
  selector: 'app-service-main-far',
  templateUrl: './service-main-far.component.html',
  styleUrls: ['./service-main-far.component.scss'],
})
export class ServiceMainFarComponent implements OnInit {
  data: any = null;
  accountId: string = '';
  ...
  async getDetails() {
    try {
      ...
      const response =
        await this.fieldactivityService.getDataToCallEstAlternateMatProcedure(
          detailsFilter
        );
  ...
  async chnageStatusToEstimation() {
    ...
    if (!this.materialCode || !this.quantity) {
      this.snackBar.open('Material Code and Quantity are required!', 'OK', {
        verticalPosition: cordova !== undefined ? 'bottom' : 'top',
    ...
    this.loader.show('Inserting Service Main Material...');
    try {
      const apiKey = sessionStorage.getItem('api-key');
      const serviceKey = sessionStorage.getItem('service-key');
      const userRole = sessionStorage.getItem('user-role');
      ...
      const detailsFilter: any = {
        ...
        userName,
        userCode,
        workorderNo: this.data?.workorderNumber,
        discom,
        materialCode: this.materialCode,
        quantity: this.quantity,
      };
      this.saveData = await this.fieldactivityService.callEstimationAltMatProc(
        detailsFilter
      );
