...
@Component({
  selector: 'app-smr-case-far',
  templateUrl: './smr-case-far.component.html',
  styleUrls: ['./smr-case-far.component.scss']
})
export class SmrCaseFarComponent implements OnInit {
  ...
  async getDetails() {
    try {
      ...
      const response = await this.fieldactivityService.getSmrDataByReferenceNumber(detailsFilter);
      ...
      if (response && response.data && response.data.length > 0) {
        ...
        this.data = null;
        this.errorMessage = 'The entered Case ID does not meet the criteria or is not associated with this category.';
      }
    } catch (error) {
  ...
  openSaveDialog(action: string): void {
    ...
    dialogRef.afterClosed().subscribe(result => {
      if (result === 'yes') {
        this.returnToSiteInspection();
      }
    });
  }
  async returnToSiteInspection() {
    if (this.isButtonDisabled) {
      return;
    }
    this.isButtonDisabled = true;
    this.loader.show('Returning to Site Inspection...');
    try {
      const apiKey = sessionStorage.getItem('api-key');
      const serviceKey = sessionStorage.getItem('service-key');
      const userRole = sessionStorage.getItem('user-role');
      const userName = sessionStorage.getItem('user-name');
      const userCode = sessionStorage.getItem('user-code');
      const detailsFilter: any = {apiKey,serviceKey,userRole,userName,userCode, caseId: this.data?.referenceNumber,discom:this.data?.discom };
      this.saveData = await this.fieldactivityService.moveBackToSiteInspection(detailsFilter);
