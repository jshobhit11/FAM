...
@Component({
  selector: 'app-back-work-execution',
  templateUrl: './back-work-execution.component.html',
  styleUrls: ['./back-work-execution.component.scss']
})
export class BackWorkExecutionComponent implements OnInit {
  data: any = null;
  accountId: string = ''; 
  ...
  openSaveDialog(action: string): void {
    ...
    dialogRef.afterClosed().subscribe(result => {
      if (result === 'yes') {
       this.putBackToWorkExecution();
      }
    });
  }
  async putBackToWorkExecution(){
    try {
      ...
      this.saveData = await this.fieldactivityService.putBackToWorkExecution(detailsFilter);
      ...
      console.error('Error occured while returning to work execution:', error);
