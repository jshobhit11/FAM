...
@Component({
  selector: 'app-amm-smr-far',
  templateUrl: './amm-smr-far.component.html',
  styleUrls: ['./amm-smr-far.component.scss']
})
export class AmmSmrFarComponent implements OnInit {
  data: any = null;
  accountId: string = ''; 
  ...
  async getDetails() {
    try {
      ...
      const response = await this.fieldactivityService.getNcAccountMeterMappingData(detailsFilter);
  ...
  async revertToMeterUpload() {
    ...
    try {
      ...
      this.saveData = await this.fieldactivityService.deactiveValidatorReplicatorForNC(detailsFilter);
