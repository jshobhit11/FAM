...
@Component({
  selector: 'app-back-meter-upload',
  templateUrl: './back-meter-upload.component.html',
  styleUrls: ['./back-meter-upload.component.scss']
})
export class BackMeterUploadComponent implements OnInit {
  data: any = null;
  accountId: string = ''; 
  ...
  openSaveDialog(action: string): void {
    ...
    dialogRef.afterClosed().subscribe(result => {
      if (result === 'yes') {
       this.workOrderCreationAtAET();
      }
    });
  }
  async workOrderCreationAtAET(){
    try {
