...
@Component({
  selector: 'app-meter-replacement-far',
  templateUrl: './meter-replacement-far.component.html',
  styleUrls: ['./meter-replacement-far.component.scss']
})
export class MeterReplacementFarComponent implements OnInit {
  data: any = null;
  accountId: string = ''; 
  ...
  async getDetails() {
    try {
      ...
      const response = await this.fieldactivityService.getMeterReplacementData(detailsFilter);
  ...
  async chnageStatusToEstimation() {
    ...
    try {
      ...
      const userName = sessionStorage.getItem('user-name');
      const userCode = sessionStorage.getItem('user-code');
      const detailsFilter: any = {apiKey,serviceKey,userRole,userName,userCode,materialSerialStockId:this.data?.materialSerialStockId,serviceRegistrationsId:this.data?.serviceRegistrationsId};
      this.saveData = await this.fieldactivityService.deactiveMeterValidatorReplicatorForMeterReplacement(detailsFilter);
