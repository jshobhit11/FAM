...
@Component({
  selector: 'app-execution-method-change',
  templateUrl: './execution-method-change.component.html',
  styleUrls: ['./execution-method-change.component.scss'],
})
export class ExecutionMethodChangeComponent implements OnInit {
  data: any = null;
  accountId: string = '';
  ...
  async getDetails() {
    try {
      ...
      const response =
        await this.fieldactivityService.getDataByWorkorderNoToUpdateExecutionMethodToSE(
          detailsFilter
        );
  ...
  openSaveDialog(action: string): void {
    ...
    dialogRef.afterClosed().subscribe((result) => {
      if (result === 'yes') {
        this.executionMethodChange();
      }
    });
  }
  async executionMethodChange() {
    try {
      const apiKey = sessionStorage.getItem('api-key');
      const serviceKey = sessionStorage.getItem('service-key');
      const userRole = sessionStorage.getItem('user-role');
      ...
      this.saveData = await this.fieldactivityService.whenDOPCategorySCW(
        detailsFilter
      );
