...
export class AwaitingPaymentComponent implements OnInit {
  ...
  async getDetails() {
    try {
      ...
      const response = await this.fieldactivityService.getDataByReferenceNoMoveToEstimationApproval(detailsFilter);
  ...
  openSaveDialog(action: string): void {
    ...
    dialogRef.afterClosed().subscribe(result => {
      if (result === 'yes') {
        this.fromAwaitingForPaymentsToEstimationApproval();
      }
    });
  }
  async fromAwaitingForPaymentsToEstimationApproval() {
    if (this.isButtonDisabled) {
      return;
    }
    this.isButtonDisabled = true;
    this.loader.show('Changing To Estimation Approval...');
    try {
      ...
      this.saveData = await this.fieldactivityService.fromAwaitingForPaymentsToEstimationApproval(detailsFilter);
