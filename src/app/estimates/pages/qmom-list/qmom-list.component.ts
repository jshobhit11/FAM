...
@Component({
  selector: 'app-qmom-list',
  templateUrl: './qmom-list.component.html',
  styleUrls: ['./qmom-list.component.scss']
})
export class QmomListComponent implements OnInit {
  estimatesList: any[] = [];
  cols: any[] = [];
  filterFields: string[] = [];
  ...
  async ngOnInit() {
    ...
    this.estimatesList = await this.estimationRegistered.getQmomEstimationApprovalList(filter);
    ...
    this.cols = [
      { key: 'estimationNo', title: 'Estimation No.', route: `/estimates/qmom-estimation-approval`},
