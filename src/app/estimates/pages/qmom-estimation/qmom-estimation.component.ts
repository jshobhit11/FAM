...
import { ViewPopUpComponent } from '../../../../app/estimate-forms/components/view-pop-up/view-pop-up.component';
...
@Component({
  selector: 'app-qmom-estimation',
  templateUrl: './qmom-estimation.component.html',
  styleUrls: ['./qmom-estimation.component.scss']
})
export class QmomEstimationComponent implements OnInit {
...
@ViewChild('categorySelect') categorySelect: MatSelect;
  ...
  async ngOnInit() {
    ...
    this.configurationService
      ...
      .then((data: any) => {
        ...
        if (selectedCharge) {
          ...
          this.onChange(selectedCharge, selectedCharge.chargesCategory);
        }
      });
    ...
    if (mode === 'edit') {
      ...
      if (estimateDetailData.estimationRegisteredDTO[0].workCategoryCode == 'SEW') {
        this.estimateCharges.get('workExecutionMethod').disable();
      }
      this.onQuantityChange;
    }
    this.workCategoryData = await this.configurationService.getWorkCategoryGetAllData(filter);
    this.workCategoryData = this.workCategoryData.filter((v: any) => (v.workCategoryCode == 'RMW') );
    ...
    //   (v: any) => v.woExecutionMethodName === 'Department' || v.woExecutionMethodName === 'Self Execution'
    // );
    ...
    this.estimateType = await this.estimateService.getEstimateTypeMasterData(filter);
    await this.onChangeMaterials();
    this.estimateType = this.estimateType.filter((v: any) => ( v.estimateTypeCode !== 'RLZ' ) );
        ...
        this.currentQuarterText = this.getQuarterlyEstimateText();
    
        this.estimateCharges.get('estimationWorkDescription')?.setValue(this.currentQuarterText);
    this.loader.hide();  
   });
  }
  getQuarterlyEstimateText(): string {
    ...
    return `Estimate prepared for quarterly maintenance for the period (${quarters[quarterIndex]} ${year})`;
  }
  ...
  async onChangeTypeOfWork(typeOfWork: string, initial?: boolean | false) {
    if (typeOfWork) {
      ...
      const workD = workDescData.filter(
        (v: any) => !['EOT', 'ELTL', 'EHTL'].includes(v.workscopeDescCode)
      );
  
      const selectedEstimate = this.estimateCharges.get('estimateType').value;
      
      const isRegular = selectedEstimate == 'RGL';
      const isCredit = selectedEstimate == 'CRD';
  
      this.workscopeDesc = workD.filter((v: any) => {
        if (isRegular) {
          return v.workTypeApplicable == 'QM'; 
        } else if (isCredit) {
          return v.workTypeApplicable == 'BOTH' && v.estimateTypeCode == 'CRD'; 
        }
        return false; 
      });
      if (this.estimateCharges.get('workPart').value && this.estimateCharges.get('workDescription').value) {
        ...
        if (alreadyExist) {
          this.snackbar.open('Scope of Work Already Exists.', 'OK', { verticalPosition: cordova !== undefined ? 'bottom' : 'top' });
      ...
      this.snackbar.open('Please select Type of Work.', 'OK', { verticalPosition: cordova !== undefined ? 'bottom' : 'top' });
      this.alreadyExistScopeOfWork = true;
    }
  }
...
calculateAmount(chargesPercentage: number, basicRate: number): number {
  ...
  async onChangeWorkCategory(workCategoryMasterId: any) {
    ...
    } else if (workCategoryMasterId == 4 ) { 
      this.workExecution = this.allWorkExecutionData.filter(
        (v: any) => v.woExecutionMethodName !== 'Self Execution'
      );
      this.estimateCharges.get('workExecutionMethod').setValue('');
  ...
  async onChangeWorkExecution(woExecutionMethodId: any) {
   const previousWorkExecutionMethodId = this.defaultWorkExecutionMethodId;
    ...
    if (this.isEditCase && woExecutionMethodId !== this.defaultWorkExecutionMethodId) {
...
async onChangeMaterials() {
  ...
  deleteMaterialItem(i: number, id: number, code: string) {
    ...
    if (code == 'RGL') {
      ...
      if (removedItem && removedItem.workscopeDescription === 'Release Labour') {
        ...
        // Handle "Credit" estimate type
          ...
          if (exists) {
            ...
            if (!selectedWorkDescription) {
              const workTableEntry = this.creditFilterExecutionWorkTable[Number(selectedRadioButton[0])];
            ...
            if (this.workscopeDesc && this.workscopeDesc.length > 0) {
              selectedWorkDescription = this.workscopeDesc.find(
                (v: any) =>
                  v.workscopeDescMasterId == Number(this.selectedWorkscopeDescriptionId)
              );
            }
            const workDescriptionName = selectedWorkDescription?.workscopeDescription;
  ...
  async onQuantityChange(
    ...
    if (Number(quantity) > 0) {
      ...
      if (isReleaseLabour) {
        ...
        mrt = 0;
      } else if (estimateTypeCode === 'CRD' && workscopeDescription === 'Release Scrap') {
  ...
  validateDecimalInput(event: KeyboardEvent): void {
    ...
    if (!inputElement.value.includes('.') && integerPart.length >= 20 && inputChar !== '.' && inputChar !== 'Backspace' && inputChar !== 'Delete') {
      event.preventDefault();
      return;
    }

    if (decimalPart && decimalPart.length >= 3 && inputChar !== 'Backspace' && inputChar !== 'Delete') {
      event.preventDefault();
    }
  }
  ...
  removeAdditionalChargesBeforeTotalLabour(estimationAdditionalChargesId: number) {
    const chargeIndex = this.addChargesBeforeTotalLabour.findIndex(
      (v: any) => v.estimationAdditionalChargesId === estimationAdditionalChargesId
    );
  
    if (chargeIndex !== -1) {
      const removedCharge = this.addChargesBeforeTotalLabour[chargeIndex];
      const isGST = removedCharge.additionalChargeName == 'GST';
      const isAreaSpecific = removedCharge.additionalChargeName == 'Area Specific Loading on Basic Rates';
  ...
  refreshAdditionalCharges() {
    const totalMLCost = Number(
      (Number(this.estimationMaterialCost) + Number(this.estimationLabourCost)).toFixed(2)
    );
    ...
    chargesBeforeLabour.forEach((charge: any) => {
      charge.amount = Number(
        ((charge.chargeTypeValue * Number(this.estimationLabourCost)) / 100).toFixed(2)
      );
    });
    ...
    const beforeTotalLabour = this.addChargesBeforeTotalLabour.reduce(
      (sum: number, charge: any) => sum + (charge.displayByDefault ? Number(charge.amount) : 0), 0
    );
    ...
    if (gstChargeIndex !== -1) {
      const gstCharge = this.addChargesBeforeTotalLabour[gstChargeIndex];
      let gstAmount;
      const isAreaSpecificLoadingPresent = this.addChargesBeforeTotalLabour.some(
        (charge) => charge.additionalChargeName === 'Area Specific Loading on Basic Rates'
      );
  
      if (isAreaSpecificLoadingPresent) {
        gstAmount = (
          (this.totalEstimateCost * gstCharge.chargeTypeValue) / 100
        ).toFixed(2);
      } else {
        gstAmount = (
          (this.basicRate * gstCharge.chargeTypeValue) / 100
        ).toFixed(2);
      }
...
addAdditionalCharges() {
  ...
  let ac = this.isTemplate
      ? this.additionalChargesSel.find(v => Number(v.estimationAdditionalChargesId) === addChargeId)
      : this.additionalChargesSel.find(v => Number(v.additionalChargesMasterId) === addChargeId);

  if (
      this.addChargesBeforeTotalLabour.some(v => Number(v.additionalChargesMasterId || v.estimationAdditionalChargesId) === addChargeId) ||
      this.addChargesAfterTotalLabour.some(v => Number(v.additionalChargesMasterId || v.estimationAdditionalChargesId) === addChargeId)
  ) {
  ...
  if (Number(ac.chargesSequenceOrder) < 5) {
      ...
      this.addChargesBeforeTotalLabour.sort((a, b) => Number(a.chargesSequenceOrder) - Number(b.chargesSequenceOrder));
  } else {
      this.addChargesAfterTotalLabour.push(ac);
      this.addChargesAfterTotalLabour.sort((a, b) => Number(a.chargesSequenceOrder) - Number(b.chargesSequenceOrder));
...
updateGSTAmount() {
  ...
  const areaSpecificLoadingIndex = this.addChargesBeforeTotalLabour.findIndex(charge => charge.additionalChargeName === 'Area Specific Loading on Basic Rates');
  if (gstChargeIndex !== -1) {
      const gstCharge = this.addChargesBeforeTotalLabour[gstChargeIndex];
      let totalBasisForGST = this.basicRate; 
      if (areaSpecificLoadingIndex !== -1) {
          const areaSpecificLoadingCharge = this.addChargesBeforeTotalLabour[areaSpecificLoadingIndex];
          totalBasisForGST += areaSpecificLoadingCharge.amount ? parseFloat(areaSpecificLoadingCharge.amount) : 0; 
      }
...
calculateTotalEstimateCost() {
  const totalBefore = this.addChargesBeforeTotalLabour.reduce((total, charge) => {
      return total + (charge.amount ? parseFloat(charge.amount) : 0);
  }, 0);
  const totalAfter = this.addChargesAfterTotalLabour.reduce((total, charge) => {
      return total + (charge.amount ? parseFloat(charge.amount) : 0);
  }, 0);
...
onChangeAdditionalCharges() {
  ...
  openSaveDialog(action: string): void {
    ...
    dialogRef.afterClosed().subscribe(result => {
      if (result === 'yes') {
        this.saveEstimation(action);
      }
    });
  }
  async submitEstimation(action:string) {
    this.loader.show('Generating Estimation...')
      const chargeData = [
      this.totalEstimateCost,
      this.totalEstimationCostInRs
    ];
    ...
    const estimationType = action === 're-generate' ? 'RG' : 'GE';
    ...
    const filter: any = { apiKey, serviceKey, userRole, userName, userCode, estimationType, isRevenueQmOmEstimation: 1 };
    const estimate: any = {
      estimationRegisteredDTO: {
        estCertificate: (this.estimatedByForm.get('certification').value||'').slice(0, this.maxCharLimits.certification),
        estReport: this.estimatedByForm.get('report').value,
        ...
        rateContractMasterId: this.estimateCharges.get('vendor').value ? Number(this.estimateCharges.get('vendor').value) : '',
    ...
    if (emptyQuantity) {
      ...
      if (res.messageType == 'SUCCESS') {
        const message = action === 'generate_bmr'
        ? 'Quarterly Maintenance Estimation Generation Done Successfully'
        : 'Quarterly Maintenance Re-Generated Estimation Successfully';
        const MatSnackBarRef = this.snackbar
          ...
          .subscribe(() => {
            this.router.navigate(['/estimates', 'qmom-estimation-approval', res.estmationRegisteredId, res.serviceRegistrationId]);
          });
      }
    }
  }
  async saveEstimation(action:string) {
      const chargeData = [
      this.totalEstimateCost,
      this.totalEstimationCostInRs
    ];
    ...
    const userName = sessionStorage.getItem('user-name');
    const userCode = sessionStorage.getItem('user-code');
    const filter: any = { apiKey, serviceKey, userRole, userName, userCode, estimationType: 'GE', isRevenueQmOmEstimation: 1 };
    const estimate: any = {
      estimationRegisteredDTO: {
        estCertificate: (this.estimatedByForm.get('certification').value||'').slice(0, this.maxCharLimits.certification),
        estReport: this.estimatedByForm.get('report').value,
    ...
    if (emptyQuantity) {
      ...
      const res = await this.estimationRegisteredService.generateEstimation(filter, estimate).catch((e: any) => console.log(e));
      if (res.messageType == 'SUCCESS') {
        const message = action === 'bmr_save'
        ? 'Estimation Saved Successfully'
        : 'Re-Generated Estimation Successfully';
        this.snackbar
          .open(message, 'OK', {
            verticalPosition: cordova !== undefined ? 'bottom' : 'top',
          })
