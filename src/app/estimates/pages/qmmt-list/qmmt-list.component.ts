...
@Component({
  selector: 'app-qmmt-list',
  templateUrl: './qmmt-list.component.html',
  styleUrls: ['./qmmt-list.component.scss']
})
export class QmmtListComponent implements OnInit {
  estimatesList: any[] = [];
  cols: any[] = [];
  filterFields: string[] = [];
  ...
  async ngOnInit() {
    ...
    this.estimatesList = await this.estimationRegistered.getQmmtEstimationApprovalList(filter);
    ...
    this.cols = [
      { key: 'estimationNo', title: 'Estimation No.', route: `/estimates/qmmt-estimation-approval`},
