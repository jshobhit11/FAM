...
@Component({
  selector: 'app-qmom-estimation-approval',
  templateUrl: './qmom-estimation-approval.component.html',
  styleUrls: ['./qmom-estimation-approval.component.scss']
})
export class QmomEstimationApprovalComponent implements OnInit {
  sh: number = 1;
  ...
  moveToEstimateForms() {
    ...
    this.router.navigate(
      [
        `/estimates/qmom-estimation/${statusCode}/${this.serviceRegistrationId}`,
      ],
      {
       ...
       this.isLoading = true;
        ...
        if (data.messageType == 'SUCCESS') {
          const snackBarRef = this.snackbar
            .open(
              'Quarterly Maintenance Estimation Forwarded Successfully',
              'OK',
              {
                verticalPosition: cordova !== undefined ? 'bottom' : 'top',
              }
  ...
  async return() {
    ...
    if (data.messageType == 'SUCCESS') {
      this.snackbar
        .open('Quarterly Maintenance Estimation Returned successfully', 'OK', {
          verticalPosition: cordova !== undefined ? 'bottom' : 'top',
        })
        .onAction()
        .subscribe(() => {
   ...
   if(this.isApproved){
    ...
    if (this.forwardData && this.forwardData.remarks) {
      ...
      if (data.messageType == 'SUCCESS') {
        const snackBarRef = this.snackbar
          .open(
            'Quarterly Maintenance Estimation Approved Successfully',
            'OK',
            {
              verticalPosition: cordova !== undefined ? 'bottom' : 'top',
            }
