...
@Component({
  selector: 'app-bmr-estimation-approval-list',
  templateUrl: './bmr-estimation-approval-list.component.html',
  styleUrls: ['./bmr-estimation-approval-list.component.scss'],
})
export class BmrEstimationApprovalListComponent implements OnInit {
  estimatesList: any[] = [];
  cols: any[] = [];
  filterFields: string[] = [];
  ...
  async ngOnInit() {
    ...
    this.estimatesList = await this.estimationRegistered.getBmrEstimationApprovalList(filter);
    ...
    this.estimatesList.sort((a, b) => {
      ...
      return dateB.getTime() - dateA.getTime();  // Sort in descending order for latest dates first
    });    
    const now = new Date();
    ...
    this.cols = [
      { key: 'estimationNo', title: 'Estimation No.', route: `/estimates/bmr-estimation-approval`},
