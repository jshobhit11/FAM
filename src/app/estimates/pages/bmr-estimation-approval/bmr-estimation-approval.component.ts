...
@Component({
  selector: 'app-bmr-estimation-approval',
  templateUrl: './bmr-estimation-approval.component.html',
  styleUrls: ['./bmr-estimation-approval.component.scss'],
})
export class BmrEstimationApprovalComponent implements OnInit {
  sh: number = 1;
  ...
  async getDivisionalBudgetData(accountHeadMasterId: any, divisionalBudgetId?: any) {
    ...
    const selectedBudget = this.budgetData.find(
      ...
      console.log('Session values:', sessionPayload);
  
      if (accountHeadMasterId) {
        console.log('Calling getDataByDivisionOfficeIdAccountHeadMasterId...');
        data = await this.estimateService.getDataByDivisionOfficeIdAccountHeadMasterId({
          ...sessionPayload,
            accountHeadMasterId,
        });
    } else if (divisionalBudgetId) {
        console.log('Calling getDataByDivisionalBudgetId...');
    ...
    if (!data) {
      ...
      data.totalBudgetAmount = Number(Number(data?.totalBudgetAmount || 0).toFixed(2));
      data.workOrderIssueAmount = Number(Number(data?.workOrderIssueAmount || 0).toFixed(2));
      data.totalReserveAmount = Number(Number(data?.totalReserveAmount || 0).toFixed(2));
      data.totalExpenditureAmount = Number(Number(data?.totalExpenditureAmount || 0).toFixed(2));
  
      data.balanceAmount = Number(
        (data.totalBudgetAmount - (data.workOrderIssueAmount + data.totalReserveAmount)).toFixed(2)
      ) || 0;
    this.divisionalBudgetData = data;
  
      const grandTotal = Number(this.grandTotal || 0);
      const actualBalanceAmount = data.totalBudgetAmount - data.totalExpenditureAmount;

      if (data.accountHeadCode === '14.400' && grandTotal > actualBalanceAmount) {
      this.isDivisionalBudgetSCValid = false;
      } else if (data.accountHeadCode !== '14.400' && grandTotal > data.balanceAmount) {
      ...
      if (data.totalBudgetAmount <= 0) {
        this.approvalErrorMessage = 'Budget is not added. Please add the budget for this Account Head!';
        this.isApproveDisabled = true;
      }
  
      // Optional: Uncomment this logic if you want to handle negative balance separately
      /*
      if (data.balanceAmount < 0 && data.accountHeadCode !== '14.400') {
        this.approvalErrorMessage = `Total Estimation Amount: ${grandTotal} is greater than Budget Actual Balance Amount: ${actualBalanceAmount}`;
        this.isApproveDisabled = true;
      }
      */
  
    } catch (error) {
      console.error("Error caught in getDivisionalBudgetData:", error);
      this.approvalErrorMessage = 'An error occurred while fetching budget data.';
      this.isApproveDisabled = true;
    ...
    console.log('Final divisionalBudgetData:', this.divisionalBudgetData);
  }
  ...
  moveToEstimateForms() {
    ...
    this.router.navigate(
      [
        `/estimates/bmr-estimation-creation/${statusCode}/${this.serviceRegistrationId}`,
      ],
      {
       ...
       this.isLoading = true;
        ...
        if (data.messageType == 'SUCCESS') {
          const snackBarRef = this.snackbar
            .open(
              'Bulk Meter Replacement Estimation Forwarded Successfully',
              'OK',
              {
                verticalPosition: cordova !== undefined ? 'bottom' : 'top',
              }
  ...
  async return() {
    ...
    if (data.messageType == 'SUCCESS') {
      this.snackbar
        .open('Bulk Meter Replacement Estimation Returned successfully', 'OK', {
          verticalPosition: cordova !== undefined ? 'bottom' : 'top',
        })
        .onAction()
        .subscribe(() => {
   ...
   if(this.isApproved){
    ...
    if (this.forwardData && this.forwardData.remarks) {
      ...
      if (data.messageType == 'SUCCESS') {
        const snackBarRef = this.snackbar
          .open(
            'Bulk Meter Replacement Estimation Approved Successfully',
            'OK',
            {
              verticalPosition: cordova !== undefined ? 'bottom' : 'top',
            }
