...
@Component({
  selector: 'app-improvement-estimation-report',
  templateUrl: './improvement-estimation-report.component.html',
  styleUrls: ['./improvement-estimation-report.component.scss'],
})
export class ImprovementEstimationReportComponent implements OnInit {
  ...
  ngOnInit() {
    this.route.paramMap.subscribe(async (params: ParamMap) => {
      ...
      const workExecutionMethod = this.data?.estimationRegisteredDTO[0].workExecutionMethod;

    // Find the corresponding workExecutionMethodCode
      const workExecutionMethodCode = this.workExecutionData.find(
        (method: any) => method.woExecutionMethodId ===Number(workExecutionMethod) 
      )?.woExecutionMethodCode;
     this.workExecutionMethodCode=workExecutionMethodCode
      ...
      if (estimationWorkScopeDataDTO) {
        estimationWorkScopeDataDTO?.forEach((estimate: any) => {
          ...
          cArr.forEach((v) => {
            if (!this.creditArray.some(existing => existing.estimate.estTemplateWorkScopeMasterId === v.estimate.estTemplateWorkScopeMasterId)) {
  ...
  async generateImprovementEstimationReport() {
