...
@Component({
  selector: 'app-bmr-estimation-creation',
  templateUrl: './bmr-estimation-creation.component.html',
  styleUrls: ['./bmr-estimation-creation.component.scss'],
})
export class BmrEstimationCreationComponent implements OnInit {
...
@ViewChild('categorySelect') categorySelect: MatSelect;
  ...
  async ngOnInit() {
    ...
    this.workCategoryData = this.workCategoryData.filter((v: any) => (v.workCategoryCode == 'IMP' || v.workCategoryCode == 'RMW'||v.workCategoryCode == 'SEW' ) );
  ...
  async onChangeTypeOfWork(typeOfWork: string, initial?: boolean | false) {
    if (typeOfWork) {
      ...
      const workD = workDescData.filter((v: any) => (v.workscopeDescCode !== 'EOT' && v.workscopeDescCode !== 'ELTL' && v.workscopeDescCode !== 'EHTL'));
...
calculateAmount(chargesPercentage: number, basicRate: number): number {
  ...
  async onChangeWorkCategory(workCategoryMasterId: any) {
    if (workCategoryMasterId == 1) {
      ...
      this.estimateCharges.get('workExecutionMethod').enable();
      this.workExecution = this.allWorkExecutionData.filter(
        (v: any) => v.woExecutionMethodName === 'Self Execution' || v.woExecutionMethodName === 'Department'
      );
    } else {
...
onChangeAdditionalCharges() {
  ...
  //   const processName='BMR Estimation Creation';
  //   try {
  ...
  async submitEstimation(action:string) {
    ...
    const userName = sessionStorage.getItem('user-name');
    const userCode = sessionStorage.getItem('user-code');
    const filter: any = { apiKey, serviceKey, userRole, userName, userCode, estimationType, isBulkmeterEstimation: 1 };
    const estimate: any = {
    ...
    if (emptyQuantity) {
      ...
      if (res.messageType == 'SUCCESS') {
        ...
        ? 'Estimation Generation Done Successfully'
        ...
        const MatSnackBarRef = this.snackbar
          ...
          .subscribe(() => {
            this.router.navigate(['/estimates', 'bmr-estimation-approval', res.estmationRegisteredId, res.serviceRegistrationId]);
          });
      }
    }
  }
  async saveEstimation(action:string) {
    ...
    const userName = sessionStorage.getItem('user-name');
    const userCode = sessionStorage.getItem('user-code');
    const filter: any = { apiKey, serviceKey, userRole, userName, userCode, estimationType: 'GE', isBulkmeterEstimation: 1 };
    const estimate: any = {
      estimationRegisteredDTO: {
        estCertificate: (this.estimatedByForm.get('certification').value||'').slice(0, this.maxCharLimits.certification),
        estReport: this.estimatedByForm.get('report').value,
