...
@Component({
  selector: 'app-qmmt-estimation',
  templateUrl: './qmmt-estimation.component.html',
  styleUrls: ['./qmmt-estimation.component.scss'],
})
export class QmmtEstimationComponent implements OnInit {
  ...
  async ngOnInit() {
    ...
    this.materialType = mt.sort((a: any, b: any) => {
      ...
      this.estimateType = await this.estimateService.getEstimateTypeMasterData(
        filter
      );
    await this.onChangeMaterials();
      this.estimateType = this.estimateType.filter(
        (v: any) => v.estimateTypeCode !== 'RLZ'
      );
      ...
      this.estimateCharges
        .get('estimationWorkDescription')
        ?.setValue(this.currentQuarterText);
    this.loader.hide();  
   });
  }
 ...
 removeAdditionalChargesAfterTotalLabour(
  ...
  async submitEstimation(action: string) {
    ...
    if (emptyQuantity) {
      ...
      if (res.messageType == 'SUCCESS') {
        const message =
          action === 'generate_bmr'
        ? 'Quarterly Maintenance MT Estimation Generation Done Successfully'
        : 'Quarterly Maintenance MT Re-Generated Estimation Successfully';
        const MatSnackBarRef = this.snackbar.open(message, 'OK', {
          verticalPosition: 'top',
        });
        MatSnackBarRef.onAction().subscribe(() => {
          this.router.navigate([
            '/estimates',
            'qmmt-estimation-approval',
  ...
  async saveEstimation(action: string) {
    this.loader.show('Saving Estimation...');
    const chargeData = [this.totalEstimateCost, this.totalEstimationCostInRs];
    const [totalEstimateCost, totalEstimationCostInRs] = chargeData;
    let emptyQuantity: boolean = false;
    let emptyAdditionalCharge: boolean = false;
    ...
    if (emptyQuantity) {
      ...
      if (res.messageType == 'SUCCESS') {
        const message =
          action === 'bmr_save'
        ? 'Quarterly Maintenance MT Estimation Saved Successfully'
        : 'Quarterly Maintenance MT Re-Generated Estimation Successfully';
        this.snackbar
          .open(message, 'OK', {
            verticalPosition: cordova !== undefined ? 'bottom' : 'top',
