...
@Component({
  selector: 'app-add-estimate-template-master',
  templateUrl: './add-estimate-template-master.component.html',
  styleUrls: ['./add-estimate-template-master.component.scss'],
})
export class AddEstimateTemplateMasterComponent implements OnInit {
  ...
  displayTemplateData(templateData: any) {
    if (templateData.estTemplateWorkScopeMaster && templateData.estTemplateWorkScopeMaster.length) {
      ...
      this.regularFilterExecutionWorkTable = [];
      this.creditFilterExecutionWorkTable = [];
      const wPart = templateData.estTemplateWorkScopeMaster.map((v: any) => v.workPart);
      wPart.forEach((v: any) => {
      ...
      templateData.estTemplateWorkScopeMaster.forEach((workScope: any) => {
        ...
        if (material && material.length) {
          material.forEach((mat: any) => {
            ...
            const labour = templateData.estimationTemplateMaterialLabourMaster.filter(
              (v: any) =>
                v.estimationWorkScopeDataId == workScope.estTemplateWorkScopeMasterId && mat.materialsMasterId == v.materialsMasterId,
            );
        ...
        if (this.estimateType.find((v: any) => v.estimateTypeMasterId == workScope.estimateType)?.estimateTypeCode == 'RGL') {
        ...
        } else if (this.estimateType.find((v: any) => v.estimateTypeMasterId == workScope.estimateType)?.estimateTypeCode == 'CRD') {
      ...
      this.estimateCharges.get('workCategory').setValue(templateData?.estimationTemplateMaster?.workCategoryMasterId);
      ...
      if (additionalCharge && additionalCharge.length) {
        ...
        this.addChargesBeforeTotalLabour = this.additionalCharges.filter((v: any) => Number(v.chargesSequenceOrder) < 5);
        this.addChargesAfterTotalLabour = this.additionalCharges.filter((v: any) => Number(v.chargesSequenceOrder) >= 5);
  ...
  async saveTemplate() {
    const estimationName = (this.estimateCharges.get('estimationWorkDescription').value||'').slice(0, this.maxCharLimits.estimationWorkDescription);
