...
export class LrLeMeterPowerApprovalComponent implements OnInit {
  ...
  async ngOnInit() {
    ...
    this.route.paramMap.subscribe(async (params: ParamMap) => {
      ...
      this.data = await this.estimationReport.getDetailsForEstimationReport(
        filter
      );
      ...
      if (this.workCategoryCode == 'SEW') {
        ...
        this.budgetControl.setValue(257);
  ...
  moveToEstimateForms() {
    this.router.navigate([
      `/main/estimate-forms/${this.statusCode}/${this.accountId}/edit/${this.processTypeName}`,
    ]);
  }
  ...
  async approve() {
    if (this.forwardData && this.forwardData.Remarks) {
      ...
      if (data.messageType == 'SUCCESS') {
        this.snackbar
          .open('Power Sanction Approved Successfully', 'OK', {
            verticalPosition: cordova !== undefined ? 'bottom' : 'top',
          })
          .onAction()
          .subscribe(() => {
