...
@Component({
  selector: 'app-service-line-estimate-approval',
  templateUrl: './service-line-estimate-approval.component.html',
  styleUrls: ['./service-line-estimate-approval.component.scss'],
})
export class ServiceLineEstimateApprovalComponent implements OnInit {
  accountId: any;
  data: any = {};
  isChecked: boolean;
  isAE: boolean = false;
  ...
  async ngOnInit() {
    ...
    console.log('this.budgetdata', this.budgetData);
    this.budgetControl.setValue(127);
    this.getDivisionalBudgetData(127);
    this.budgetControl.disable();
  }
  ...
  moveToEstimateForms() {
    this.router.navigate([
      `/main/service-line-estimate/${this.serviceRegistrationsId}/${this.statusCode}/${this.processTypeName}/edit`,
    ]);
  }
      ...
      if (this.forwardData && this.forwardData.Remarks) {
        ...
        if (data.messageType == 'SUCCESS') {
          const snackBarRef = this.snackbar
            .open('Service Line Estimate Forwarded Successfully', 'OK', {
              verticalPosition: cordova !== undefined ? 'bottom' : 'top',
            })
  ...
  async return() {
    ...
    if (data.messageType == 'SUCCESS') {
      this.snackbar
        .open('Service Line Estimate Returned Successfully', 'OK', {
          verticalPosition: cordova !== undefined ? 'bottom' : 'top',
        })
        .onAction()
        .subscribe(() => {
  ...
  async approve() {
    ...
    if (this.forwardData && this.forwardData.Remarks) {
      ...
      //     estimationRegisteredId:
      //       this.data.estimationRegisteredDTO[0]?.estimationRegisteredId,
      //     accountHeadMasterId: 3151,
      //     divisionalBudgetId: 0,
      //     totalBudgetAmount: 0,
      //     workOrderIssueAmount: 0,
      //     balanceAmount: 0,
      //   };
      // } else
       if (this.isBudgetDetails) {
      ...
      this.isLoading = true;
      console.log('data for the budget before save', filter);
      const data = await this.estimateService.approvalForServiceMainEstimates(
        filter,
        wet
      );
      this.forwardData = {};
      if (data.messageType === 'SUCCESS') {
        const snackBarRef = this.snackbar.open('Service Line Estimate Approved Successfully', 'OK', {
            verticalPosition: cordova !== undefined ? 'bottom' : 'top',
          })
        ...
        const snackFailBar = this.snackbar.open(
          data.messageText || 'Approval failed. Please try again.',
          'OK',
          {
            verticalPosition: cordova !== undefined ? 'bottom' : 'top',
        }
