...
@Component({
  selector: 'app-work-management-dashboard',
  templateUrl: './work-management-dashboard.component.html',
  styleUrls: ['./work-management-dashboard.component.scss'],
})
export class WorkManagementDashboardComponent implements OnInit {
  sideBarOpen = true;
  data: any;
  data2: any;
  data3: any;
  ...
  async ngOnInit() {
    ...
    this.data = await this.dashboardService.GetWorksPendingIndentApplicationsForUser(
      mainCategoryFilter
    );

  //console.log('this.Count DATA======',this.data );

    const labels = this.data.map(item => item.statusName);
