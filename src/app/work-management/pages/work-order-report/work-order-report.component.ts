...
export class WorkOrderReportComponent implements OnInit {
  ...
  getPdf() {
    ...
    const userName = sessionStorage.getItem('user-name');
    const userCode = sessionStorage.getItem('user-code');
    const officeId = this.officeCode;
    const fromDate = this.formatDate(this.formGroup.value.fromDate);
    const toDate = this.formatDate(this.formGroup.value.toDate);
    return this.http.get(
      `${environment.baseURL}/api/workOrder/generateWorkorderApprovedListReport?userName=${userName}&userCode=${userCode}&userRole=${userRole}&apiKey=${apiKey}&serviceKey=${serviceKey}&officeId=${officeId}&fromDate=${fromDate}&toDate=${toDate}`,
  ...
  getExcelFile() {
    ...
    const toDate = this.formatDate(this.formGroup.value.toDate);
    return this.http.get(
      `${environment.baseURL}/api/workOrder/generateWorkorderApprovedListReportXLS?userName=${userName}&userCode=${userCode}&userRole=${userRole}&apiKey=${apiKey}&serviceKey=${serviceKey}&officeId=${officeId}&fromDate=${fromDate}&toDate=${toDate}`,
      { observe: 'response', responseType: 'blob' },
    );
  }
  ...
  downloadExcelFile() {
    this.getExcelFile().subscribe((fileData: any) => {
      ...
      if (link.download !== undefined) {
        ...
        link.setAttribute('download', `Work Order Report.xlsx`);
