...
@Component({
  selector: 'app-material-return-acknowledgement',
  templateUrl: './material-return-acknowledgement.component.html',
  styleUrls: ['./material-return-acknowledgement.component.scss']
})
export class MaterialReturnAcknowledgementComponent implements OnInit {
  ...
  async ngOnInit() {
    this.route.paramMap.subscribe(async (params: ParamMap) => {
      ...
      if (this.data && this.data.wmMaterialsIndentTransLog && this.data.wmMaterialsIndentTransLog.length > 0) {
        this.workscopeDescCode = this.data.wmMaterialsIndentTransLog[0].workscopeDescCode;
      } else {
  ...
  getPdf() {
    ...
    const workscopeDescCode=this.workscopeDescCode;
    return this.http.get(
      `${this.apiUrl}/api/materialInvoice/generateMaterialAcknowledgementReport?userName=${userName}&userCode=${userCode}&userRole=${userRole}&apiKey=${apiKey}&serviceKey=${serviceKey}&wmMaterialsIndentId=${wmMaterialsIndentId}&workscopeDescCode=${workscopeDescCode}`,
