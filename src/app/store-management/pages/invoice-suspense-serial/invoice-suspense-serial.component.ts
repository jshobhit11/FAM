...
import { InvoiceStoreSuspenseDetailsComponent } from '../dispatch-material-invoice/pages/dispatch-material-invoice-details/material-indent-invoice-report/invoice-store-suspense-details/invoice-store-suspense-details.component';

@Component({
  selector: 'app-invoice-suspense-serial',
  templateUrl: './invoice-suspense-serial.component.html',
  styleUrls: ['./invoice-suspense-serial.component.scss']
})
export class InvoiceSuspenseSerialComponent implements OnInit {
  ...
  constructor(
    ...
    public dialogRef: MatDialogRef<InvoiceStoreSuspenseDetailsComponent>,
    ...
    await this.updateSerialNumbers();
        this.newCurrentQty = this.data.newCurrentQty;
    this.usedCurrentQty = this.data.usedCurrentQty;
    if (this.data.serialData) {
      const serialNumbers = this.data.serialData.split(',');
      // console.log('==========serialNumbers=====',serialNumbers);
      this.displaySerialData(serialNumbers, this.invoiceData);
    }
  ...
  displaySerialData(serialNumbers: string[], invoiceData: any) {
    serialNumbers.forEach(serialNo => {
      const invoice = invoiceData.find(item => item.serialNo === serialNo);
      
      if (invoice) {
        const newItem = {
          isSelected: true,
          serialNo: serialNo,
          badgeNumber: invoice.badgeNumber || '', 
          materialStatus: this.selectedType === 'new' ? 'new' : 'used',
          materialSerialStockId: invoice.materialSerialStockId || null, 
  ...
  async updateSerialNumbers() {
    ...
    if (this.selectedType === 'new') {
      this.invoiceData =
        await this.materialInvoiceService.getSerializeMaterialsByMaterialMasterIdAndStoreOfficeId(
          filter
        );
    } else if (this.selectedType === 'used') {
      this.invoiceData =
        await this.materialInvoiceService.getSerializeMaterialsByMaterialMasterIdAndStoreOfficeIdUsed(
          filter
        );
    }
  ...
  async onApply() { 
    if (this.newCurrentQty == this.newSerialNumbers.length && this.usedCurrentQty == this.usedSerialNumbers.length) {
  
    this.dialogRef.close(this.addSerialNo);
    }

   else if (
    (this.newCurrentQty == this.newSerialNumbers.length || this.usedCurrentQty == this.usedSerialNumbers.length) 

  ) {
    const currentQty =
      this.selectedType === 'new' ? this.newCurrentQty : this.usedCurrentQty;
    const config: MatSnackBarConfig = {
      verticalPosition: cordova !== undefined ? 'bottom' : 'top',
      duration: 3000,
    };
    this.snackbar.open(
      `You should add up to ${currentQty} serial numbers.`,
      'Close',
      config
    );
  }
  ...
  addSNo() {
    ...
    const isDuplicate =
      this.newSerialNumbers.some((item) => item.serialNo === sno) ||
    ...
    if (isDuplicate) {
      ...
      this.snackbar.open(
        'Duplicate serial number. Please choose a different one.',
        'Close',
        config
      );
    } else if (
      currentQty > 0 &&
      this.newSerialNumbers.length + this.usedSerialNumbers.length >= currentQty
    ) {
          const config: MatSnackBarConfig = {
        verticalPosition: cordova !== undefined ? 'bottom' : 'top',
        duration: 3000,
      ...
      this.snackbar.open(
        `You can only add up to ${currentQty} serial numbers.`,
        'Close',
        config
      );
    } else {
      const invoice = this.invoiceData.find((v: any) => v.serialNo === sno);
      const newItem = {
        isSelected: true,
        serialNo: sno,
